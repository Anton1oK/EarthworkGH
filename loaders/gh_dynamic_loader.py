...
# The project folder is auto-detected from the connected script File Path, so the
# loader is portable: copy the folder anywhere and point the File Path at one of
# the gh_components scripts. PROJECT_FOLDER_FALLBACK is used only when the path
# cannot be read yet; set it if you prefer a fixed location.
PROJECT_FOLDER_FALLBACK = ""


def _peek_first_input():
    """Read the first GH input (the File Path) before the rest of the loader."""
...
def _detect_project_folder():
    candidate = (
        globals().get("script_path")
        or globals().get("x")
        or _peek_first_input()
    )
    if candidate:
        path = str(candidate).strip('"')
        if os.path.isfile(path):
            folder = os.path.dirname(os.path.abspath(path))
            if os.path.basename(folder).lower() == "gh_components":
                return os.path.dirname(folder)
            return folder
    return PROJECT_FOLDER_FALLBACK


PROJECT_FOLDER = _detect_project_folder()

if PROJECT_FOLDER and PROJECT_FOLDER not in sys.path:
    sys.path.append(PROJECT_FOLDER)
GH_COMPONENTS_FOLDER = os.path.join(PROJECT_FOLDER, "gh_components") if PROJECT_FOLDER else ""
if GH_COMPONENTS_FOLDER and GH_COMPONENTS_FOLDER not in sys.path:
...
def _candidate_script_path():
    ...
    component_path = os.path.join(GH_COMPONENTS_FOLDER, os.path.basename(path))
    if os.path.exists(component_path):
        return component_path

    raise ValueError(
        "Script file does not exist: {}\n"
        "Also checked: {}".format(path, component_path)
    )
